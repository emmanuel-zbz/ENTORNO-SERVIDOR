...
public class FabricantesServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (pathInfo == null || "/".equals(pathInfo)) {
            ...
            Comparator<FabricanteDTO> comparador = Comparator.comparing(Fabricante::getNombre);


            if ("codigo".equals(ordenarPor)) {
                 comparador = Comparator.comparing(Fabricante::getIdFabricante);
            }



            if ("desc".equals(modoOrdenar)) {
			...
			List<FabricanteDTO> listaFabricantesDTO = listaFabricantes.stream()
							.map(f -> new FabricanteDTO(f.getIdFabricante(), f.getNombre(), fabDAO.getCountProductos(f.getIdFabricante())))
                            .sorted(comparador)
									.toList();
